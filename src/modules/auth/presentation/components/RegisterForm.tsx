...
export const RegisterForm = ({ step, setStep }: RegisterFormProps) => {
    const { fields, errors, toast, handleChange, handleSubmit, validate } = useRegisterForm()
                ...
                <form onSubmit={handleSubmit} noValidate className="p-4 pt-0">
                    {step === 1 && (
                        <div className="animate-fadeIn">
                            <Input label="Correo electrónico" type="email" placeholder="nombre@dominio.com" value={fields.email} onChange={handleChange('email')} error={errors.email} />
                            <Input label="Contraseña" type="password" value={fields.password} onChange={handleChange('password')} error={errors.password} />
                            <Input label="Confirmar Contraseña" type="password" value={fields.confirmPassword} onChange= {handleChange('confirmPassword')} error={errors.confirmPassword} />
                            ...
                            <button onClick={nextStep} className="mt-4 w-full py-2 rounded-lg text-sm text-white font-light bg-[#8781fa] hover:bg-[#6960ec] transition-colors">Siguiente</button>
                        </div>
                    )}

                    {step === 2 && (
                        <div className="animate-fadeIn">
                            <Input label="Nombre/s" type="text" placeholder="Nombre/s" value={fields.nombre} onChange={handleChange('nombre')} error={errors.nombre} />
                            <Input label="Apellidos" type="text" placeholder="Apellidos" value={fields.apellido} onChange={handleChange('apellido')} error={errors.apellido} />
                            <div className="flex gap-3 mt-6">
                                <button type="button" onClick={prevStep} className="w-1/3 py-1 rounded-lg text-[#6C63FF] font-light text-sm border border-[#6C63FF] hover:bg-gray-50 transition-colors"> Atrás</button>
                                <button onClick={nextStep} className="w-2/3 py-2 text-sm rounded-lg text-white font-light bg-[#8781fa] hover:bg-[#6960ec] transition-colors"> Siguiente </button>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 3 && (
                        <div className="animate-fadeIn">
                            <Input label="Profesión"  select={true} placeholder="Selecciona tu profesión" options={PROFESIONES} value={fields.profesion} onChange={handleChange('profesion')} error={errors.profesion}/>
                            <Input label="Biografía" textArea={true} placeholder="Cuéntanos un poco sobre ti..." value={fields.biografia} onChange={handleChange('biografia')} error={errors.biografia}/>
                            <div className="flex gap-3 mt-4">
                                <button type="button" onClick={prevStep} className="w-1/3 py-1 rounded-lg text-[#6C63FF] font-light text-sm border border-[#6C63FF] hover:bg-gray-50 transition-colors">Atrás</button>
                                <button type="submit" className="w-2/3 py-2 text-sm rounded-lg text-white font-light bg-[#8781fa] hover:bg-[#6960ec] transition-colors">Finalizar registro</button>
                            </div>
                        </div>
                    )}
