...
export function UserReportModal({ isOpen, onClose, repository }: UserReportModalProps) {
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    try {
      await repository.downloadUserReport({ desde: finalDesde, hasta: finalHasta, estado });
      onClose();
    } catch (error: any) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#0f111a] w-full max-w-lg rounded-[24px] border border-white/10 p-8 shadow-2xl relative">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-white text-xl font-bold">Reporte de usuarios</h2>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-white text-sm font-medium mb-2">Estado de cuenta</label>
            <div className="relative">
              <select
                value={estado}
