  ...
  return (
    <div className="overflow-y-auto flex flex-col gap-5 px-6 py-5 scrollbar-thin scrollbar-thumb-[#2c2f48] flex-1">
      ...
      <div className="pb-4 border-b border-white/5">
        <h3 className="text-white font-bold text-lg">{complaint.portfolioTitle}</h3>
        <div className="flex items-center gap-3 mt-2">
          ...
          <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_COLOR[complaint.status]}`}>
            {complaint.status}
          </span>
        </div>
      </div>
      ...
      {/* Usuario */}
      <div>
        <p className="text-[10px] font-bold uppercase tracking-widest text-[#5a6278] mb-2">Propietario</p>
        <div className="flex items-center gap-2">
          <span className="text-white text-sm font-medium">{complaint.ownerUserName}</span>
      ...
      <div>
        ...
        <div className="space-y-2.5">
          {complaint.complaints.map((c) => (
            <div key={c.id} className="bg-[#171B28] rounded-xl border border-white/5 px-4 py-3">
...
export function ViewComplaintsModal({ isOpen, onClose, complaints }: ViewComplaintsModalProps) {
  const [statusFilter, setStatusFilter] = useState<'ALL' | 'pendiente' | 'revisado'>('ALL');
  ...
  const filtered = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter((c) => c.portfolioTitle.toLowerCase().includes(term));
    }
    if (statusFilter !== 'ALL') {
  ...
  const sorted = useMemo(() => {
    return [...filtered].sort((a, b) => b.complaints.length - a.complaints.length);
  }, [filtered]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
      ...
      >
        ...
        <div className={`flex flex-col overflow-hidden transition-all duration-300 ${selectedId ? 'w-full md:w-80 shrink-0' : 'w-full'}`}>
          {/* Header */}
          <div className="flex items-center justify-between px-8 pt-8 pb-5 border-b border-white/5 flex-shrink-0">
            <div>
              <h2 className="text-white text-xl font-bold">Portafolios denunciados</h2>
              <p className="text-[#9ca3af] text-xs mt-0.5">{filtered.length} portafolio{filtered.length !== 1 ? 's' : ''}</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="px-6 pt-5 pb-3 flex items-end gap-3 flex-shrink-0">
            <div className="flex-1 flex flex-col gap-1.5">
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[#5a6278]"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
                </div>
                <input
                  type="text"
            ...
            <div className="w-32 flex flex-col gap-1.5 shrink-0">
              <label className="text-[9px] uppercase font-bold tracking-wider text-[#5a6278]">Estado</label>
              <select
                value={statusFilter}
            ...
            {paginated.length === 0 ? (
              <div className="text-center flex flex-col items-center p-8 gap-3">
                <div className="w-12 h-12 rounded-2xl bg-[#7c6bec]/10 flex items-center justify-center">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-[#7c6bec]"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
                </div>
                <p className="text-white font-semibold text-sm">Sin resultados</p>
              </div>
            ) : (
              paginated.map((c) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${isSelected ? 'bg-[#7c6bec]/30 text-[#C9BEFF]' : 'bg-[#7c6bec]/15 text-[#8e80f5]'}`}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-sm font-bold truncate">{c.portfolioTitle}</p>
                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full mt-0.5 inline-block ${STATUS_COLOR[c.status]}`}>
                        {c.status}
                      </span>
                    </div>
                    <button
                      ...
                      className={`p-2 rounded-xl border transition-all shrink-0 ${
                        isSelected ? 'bg-[#7c6bec]/20 border-[#7c6bec]/40 text-[#C9BEFF]' : 'border-white/10 text-[#9ca3af] hover:text-white hover:border-white/20'
                      }`}
                    >
                      {isSelected ? (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-white/5 flex-shrink-0">
              ...
              <span className="text-[#9ca3af] text-xs font-medium">{currentPage} / {totalPages}</span>
              <button
          ...
          <div className="flex flex-col flex-1 border-t md:border-t-0 md:border-l border-white/5 overflow-hidden rounded-b-[24px] md:rounded-b-none md:rounded-r-[24px] min-w-0 animate-fade-in">
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-white/5 flex-shrink-0">
              <div className="flex items-center gap-2 text-[#C9BEFF]">
                ...
                <span className="text-xs font-bold uppercase tracking-wider">Detalle</span>
              </div>
              <button onClick={() => setSelectedId(null)} className="p-1.5 rounded-lg border border-white/5 text-[#9ca3af] hover:text-white">
